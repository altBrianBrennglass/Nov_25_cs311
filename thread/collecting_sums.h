...
sum[turbine->col][turbine->row] = 0;
   for(i = 0; i < 4; i++){
                if(!(neighbors[turbine->col][turbine->row][i][0] < 0)){//if the relevent neighbor exists
				  ...
				  //printf("[%i][%i] | neighbor [%i][%i] = %f\n", turbine->col, turbine->row,neighbors[turbine->col][turbine->row][i][0]
																		 //   ,neighbors[turbine->col][turbine->row][i][1],
																		//	current_pow[neighbors[turbine->col][turbine->row][i][0]][neighbors[turbine->col][turbine->row][i][1]]);
